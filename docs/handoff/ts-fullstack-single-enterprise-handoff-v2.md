...
- S8 只补消息中心、轻量审批和前端当前页 CSV 导出，不做复杂审批流。
...
- README 和企业交付文档已有基础。
- S8 已完成消息中心、审批请求、主要表格当前页 CSV 导出。
- OpenAPI 已改为 NestWeb 源码生成固定契约，并在前后端 CI 中做 drift 检查。

### 1.2 当前仍需完善的问题

截至 2026-06-09，当前单企业后台基础版已经完成 S1 / S2 / S3 / S5 / S7 / S8 范围，可以作为业务二开的起点。

当前剩余内容分为两类：

- 明确暂停：S4 知识库 MVP、S6 AI 知识助手。
- 未来专项：复杂审批流、后端异步全量导出、消息偏好、刷新 token 复用检测、多租户、Department / Position。

这些都不是当前交付缺口，只有出现明确产品需求时才进入新阶段。
...
- S2 已完成：账号安全、角色能力包、个人中心、修改密码、登录安全能力已落地。
- S3 已完成：系统配置、字典、文件中心已落地。
- S5 已完成：审计运维、登录日志、系统状态、版本信息、队列状态已落地。
- S7 已完成：错误页、空状态、E2E、交付文档已收口。
- S8 已完成：消息中心、审批基础预留、前端当前页 CSV 导出已落地。
- OpenAPI 已完成源码生成固定契约，前后端 CI 都检查生成结果是否漂移。

本轮路线调整：
- Department / Position 仅作为未来可选扩展。
- Role 作为岗位 / 职责 / 能力包。
- 当前主线已完成到 S8；S4 知识库和 S6 AI 继续暂停，后续作为专项重新评估。
