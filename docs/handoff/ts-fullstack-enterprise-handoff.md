...
当前已经完成到阶段 4 的核心范围，并进入 P5 前的单租户企业交付收口：

- P1：Role.code 权限模型收口已完成，系统身份使用 `Role.code`，`name` 只做显示名。
- P2：Antdpro6 前端企业化模板产品化已完成核心，默认首页切到 `/dashboard`。
- P3：Playwright 自动化 E2E 已补基础认证链路。
...
- P4 补充：初始管理员 seed 已改为 `ADMIN_EMAIL`/`ADMIN_USERNAME`/`ADMIN_PASSWORD` 驱动，生产禁止默认弱口令。
