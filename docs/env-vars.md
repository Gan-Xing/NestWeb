...
| Variable     | Purpose                                        | Default                                 |
| ------------ | ---------------------------------------------- | --------------------------------------- |
| `COMMIT_SHA` | Commit SHA shown by `/api/system/version`      | Falls back to `GIT_COMMIT` or `unknown` |
| `GIT_COMMIT` | Alternate commit SHA source                    | `unknown`                               |
| `BUILD_TIME` | Build timestamp shown by `/api/system/version` | Falls back to `BUILD_DATE` or `unknown` |
| `BUILD_DATE` | Alternate build timestamp source               | `unknown`                               |
